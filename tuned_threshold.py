...
def init_models(config, device, model_num):
    ...
    if config.include_text:
        fusion_layer.load_state_dict(torch.load(os.path.join(config['model_path'],
                                                             "fusion_model_{}".format(model_num)),
                                                map_location=device))
    fusion_layer.eval()
...
if __name__ == '__main__':
    ...
    if config.include_graph:
        ...
        graph_embeddings_dev = load_stored_embeddings(config, split='dev')
    ...
    for num in range(10):
        ...
        span_repr, span_scorer, pairwise_scorer, fusion_model = init_models(config, device, num)
        ...
        for i, predicted in enumerate(clusters):
            ...
            if not config['use_gold_mentions']:
                all_clusters, new_doc_ids, new_starts, new_ends = remove_nested_mentions(all_clusters, doc_ids, starts,
                                                                                         ends)
            else:
            ...
            write_output_file(data.documents, all_clusters, new_doc_ids, new_starts, new_ends, config['save_path'],
                              doc_name,
